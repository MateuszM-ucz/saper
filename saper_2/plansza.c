...
void inicjalizuj_plansze(Plansza *plansza, int wiersz, int kolumna, int mina, int sx, int sy){
    ...
    plansza->komórki = malloc(wiersz * sizeof(int *));
    plansza->widocznosc = malloc(wiersz * sizeof(char *));
    for(int i=0; i<wiersz; i++){
        plansza->komórki[i] = calloc(kolumna, sizeof(int));
    ...
    for(int i=0; i<mina; i++){
        ...
        do{
            x = rand() % wiersz;
            y = rand() % kolumna;
        } while((x == sx && y == sy) || plansza->komórki[x][y] == -1);
        plansza->komórki[x][y] = -1;
        ...
        for(int dx = -1; dx <= 1; dx++){
            for(int dy = -1; dy <= 1; dy++){
                int nx = x + dx;
                int ny = y + dy;
                if(nx >= 0 && ny >= 0 && nx < wiersz && ny < kolumna && plansza->komórki[nx][ny] != -1){
                    plansza->komórki[nx][ny]++;
                }
            }
        }
    }
...
int sprawdz_wygrana(const Plansza *plansza) {
    for (int i = 0; i < plansza->wiersz; i++) {
        for (int j = 0; j < plansza->kolumna; j++) {
            ...
            if (plansza->komórki[i][j] != -1 && plansza->widocznosc[i][j] == '#') {
...
void zwolnij_plansze(Plansza *plansza) {
    for (int i = 0; i < plansza->wiersz; i++) {
        free(plansza->komórki[i]);
        free(plansza->widocznosc[i]);
    }
    free(plansza->komórki);
